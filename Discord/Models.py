...
class MyRecruSelect(discord.ui.View):
    select_options = [discord.SelectOption(label="Cała rekrutacja", value=3), 
    ...
    @discord.ui.select(placeholder = "Sposób Rekrutacji", options=select_options)
...
class Form1(FormAddPlayer):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(f'Formularz został wysłany. Na {interaction.guild.get_channel_or_thread(self.recru_channel_log_id).mention} zobacz podsumowanie.', ephemeral=True)
...
class Form2(FormAddPlayer):
    def __init__(self, interaction, choice):
        super().__init__(title='Formularz', interaction=interaction, choice=choice)
        self.in_house = ui.TextInput(label='Czy w rodzie?', placeholder="1/2/3/nie", required=False)
